...
Router(config-if)#ipv6 address 2001:BD8:1::1/64
...
``` cisco ios
Router#ping 2001:BD8:1::10

Type escape sequence to abort.
Sending 5, 100-byte ICMP Echos to 2001:BD8:1::10, timeout is 2 seconds:
!!!!!
Success rate is 100 percent (5/5), round-trip min/avg/max = 0/0/1 ms
```

Ahora configuramos DHCPv6 para las redes LAN.

Cremos un POOL de direcciones con el prefijo 2001:DB8:1100::/48, para asignar redes /64. Con lo que conseguimos que cada red tenga un rango /64.

``` cisco ios
ISP(config)#ipv6 local pool LOCAL_POOL 2001:DB8:1100::/40 48
```

Ahora configuramos el servidor DHCP:

``` cisco ios
ISP(config)#ipv6 dhcp pool SUBNETES
ISP(config-dhcpv6)#prefix-delegation pool LOCAL_POOL
...
Esto le dice al router que tenemos un grupo llamado LOCAL_POOL y que podemos usar todo el prefijo 2001:DB8:1100::/40. Sin embargo, cada cliente DHCP recibirá un prefijo /48 de este grupo.
...
ISP(config-if)#ipv6 dhcp server SUBNETES
...
DHCPv6 pool: SUBNETES
  Prefix pool: LOCAL_POOL
...
La salida anterior nos dice que tenemos dos clientes y que el router ISP utiliza el LOCAL_POOL para el prefijo. Echemos un vistazo más de cerca a esos dos clientes DHCP:
